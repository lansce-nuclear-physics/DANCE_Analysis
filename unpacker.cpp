...
int Unpack_Data(gzFile gz_in, double begin, int runnum) {
  ...
  while(run) {
    ...
    if(TOTAL_EVTS > progresscounter*ProgressInterval) {
      progresscounter++;
      if(timesort) {
	cout<<"oldest time in deque: "<<datadeque[0].timestamp<<endl;
	cout<<"newest time in deque: "<<datadeque[datadeque.size()-1].timestamp<<endl;
      }	
	...
	while(TotalBankSize>0) {
	  ...
	  if (bank32.fName[0]=='C' && bank32.fName[1]=='E') {	// name starts as CE VT_BANK
		...
		double temp_slow = db_arr[EVTS].lgate + gRandom->Uniform(0,1);
		double temp_fast = db_arr[EVTS].sgate + gRandom->Uniform(0,1);
		
		db_arr[EVTS].Eslow = 0.001*(temp_slow*temp_slow*slow_quad[db_arr[EVTS].ID] +
					    temp_slow*slow_slope[db_arr[EVTS].ID] +
					    slow_offset[db_arr[EVTS].ID]);
		
		db_arr[EVTS].Efast = 0.001*(temp_fast*fast_slope[db_arr[EVTS].ID] +
					    fast_offset[db_arr[EVTS].ID]);
	      }
	      
	      if(HAVE_Threshold==1) {
		if(db_arr[EVTS].Eslow < Energy_Threshold) {
		  db_arr[EVTS].Valid=0;
		}
	      }


	      //dont do work on and put invalid events in the timesorter or eventbuilder
	      if(db_arr[EVTS].Valid == 1) {
		...
		if(EVTS >= BlockBufferSize) {
		  ...
		  for(int j=0; j<EVT_SORT; j++) {
		    ...
		    //Apply blocking time to dance crystals
		    if(db_arr[j].ID < 162) {
		      if(db_arr[j].TOF < (last_timestamp[db_arr[j].ID] + Blocking_Time)) {
			db_arr[j].Valid=0;
			Events_Invalid++;
		      }
		    }
