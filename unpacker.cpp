...
// added on Jan 7,2015  MJ
  ...
  int m1,m2,m3,m4;
  cout << "--> Reading a DANCE Map in" << endl;
...
int Unpack_Data(gzFile gz_in, double begin, int runnum) {
  ...
  while(run) {
   ...
   if(TOTAL_EVTS > EventLimit) {
	...
	cout<<"newest time in deque: "<<datadeque[datadeque.size()-1].timestamp<<endl;
      ...
      cout<<Events_Sent<<" Events_Sent to Analyzer"<<endl;
	...
	if (bank32.fName[0]=='C' && bank32.fName[1]=='E') {	// name starts as CE VT_BANK
	  ...
	  for (uint32_t evtnum=0;evtnum<evaggr->N;++evtnum) {
	    ...
	    //	    hID->Fill(db_arr[EVTS].board*16+db_arr[EVTS].channel);
  ...
  cout<<endl<<TOTAL_EVTS<<" Events Processed "<<endl;
  //	cout<<Events_Sorted<<" Events Sorted"<<endl;
  cout<<Events_Sent<<" Events_Sent to Analyzer"<<endl;
  cout<<Events_Analyzed<<" Events Analyzed"<<endl;
